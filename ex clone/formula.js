...
// for (let i = 0; i < Allcells.length; i++) {
//     Allcells[i].addEventListener("blur", function () {   //blur event saves the text of cell after another cell is selected
//         //db->value
//         let { rid, cid } = getRIdIdfromAddress();
//         let cellObj = sheetArr[rid][cid];
...
//         }
//         cellObj.value = Allcells[i].innerText;
//         //telling children to revaluate and update themselves
//         updateChildren(cellObj);
//         if(cellObj.formula){
//             removeFormula(addressElem.value,cellObj,cellObj.formula);
//         }
...
//     for(let i=0;i<children.length;i++){
//         let child= children[i];
//         let cCid = Number(child.charCodeAt(0)) - 65;   //column
//         let cRid = Number(child.slice(1)) - 1;    //row
//         let cFormula = sheetArr[cRid][cCid].formula;       
//         let value=evaluateFormula(cFormula);
//         setCell(value, cRid, cCid,cFormula);  // changes on ui and db 
//         updateChildren(sheetArr[cRid][cCid]);
//     }
// }

// function removeFormula(myName,cellObj,cFormula){
//     let formulaTokens = cFormula.split(" ");  //to get 
...
//             // A->z
//             // 1->100
//             let pCid = Number(parentCell.charCodeAt(0)) - 65;   //column
//             let pRid = Number(parentCell.slice(1)) - 1;    //row
//             let parentObj = sheetArr[pRid][pCid];
//             let idx=parentObj.children.indexOf(myName);
//             parentObj.children.splice(idx,1);
//         }
